...
export class CaseDetailsComponent implements OnInit, OnDestroy {
  ...
  getPendingWithDisplay(): string {
    ...
    if (Array.isArray(names) && names.length)
      return names.map((n: string) => String(n)).join(', ');
    return '';
  }
  ...
  canResubmit(): boolean {
    ...
    const name = (
      (this.case as any).currentStateName ||
      (this.case as any).statusName ||
      ''
    ).toLowerCase();
    return (
  ...
  get formDataGrouped(): FormDataGroup[] {
    ...
    const sortedGroupKeys = Array.from(byGroup.keys()).sort(
      (a, b) =>
        (groupLabelOrder.get(a) ?? 999) - (groupLabelOrder.get(b) ?? 999),
    );
    for (const fieldGroup of sortedGroupKeys) {
      ...
      for (const i of items) {
        const id = (i.fieldName || i.fieldLabel || '')
          .trim()
          .toLowerCase()
          .replace(/\s+/g, ' ');
        if (id) {
